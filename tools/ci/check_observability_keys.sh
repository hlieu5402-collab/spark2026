...
#
# 目标（Why）：
# - 保证 `contracts/observability_keys.toml` 与生成产物 `spark-core/src/observability/keys.rs`、
...
if ! git diff --quiet -- spark-core/src/observability/keys.rs; then
  echo "可观测性键名生成文件与合约不同步：请运行构建并提交 \"spark-core/src/observability/keys.rs\"。" >&2
  exit 1
fi
