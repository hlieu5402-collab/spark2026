...
//!
//! # 教案背景（Why）
//! - 任务 T24 要求在**不修改核心 crate** 的前提下新增扩展，本文档示例演示如何编写遵循 `spark-core` 契约的外部 Codec；
...
//!
//! # 合约说明（What）
//! - 该扩展完全依赖 `spark-core` 的公开稳定接口，不需要启用核心 crate 的 `std` 功能即可构建；
//! - 所有错误码遵循 `spark_core::error::codes` 约定，便于在指标、日志中统一聚合；
