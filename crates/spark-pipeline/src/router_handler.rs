...
//! - **逻辑 (How)**：模块内部不重新实现逻辑，而是 `pub use` `spark_router::pipeline::RouterHandler`
//!   作为兼容层；这样可以共享 `spark-router` 的完整行为与测试覆盖。
//! - **契约 (What)**：调用方需提供与 `spark_router::pipeline::RouterHandler` 相同的上下文和
//!   生命周期约束；本模块不引入额外状态，也不承担初始化职责。
//! - **注意事项 (Trade-offs & Gotchas)**：兼容层牺牲了一部分命名的显式性，但换来路径
//!   稳定性；未来若彻底移除旧接口，应在发布说明中明确告知并引导迁移到新的别名。

pub use spark_router::pipeline::RouterHandler;
